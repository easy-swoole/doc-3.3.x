---
title: easyswoole协程http客户端
meta:
  - name: description
    content: easyswoole协程http客户端
  - name: keywords
    content: easyswoole协程http客户端|swoole协程http客户端|swoole协程curl
---


## HTTPClient
协程httpClient组件,基于swoole http client实现,可在协程内发起http请求不被阻塞,可用于下载文件,请求api,爬虫等一系列需求当中
